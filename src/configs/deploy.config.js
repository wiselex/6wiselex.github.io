...
export default {
	...
	messages: {
		...
		deploy: {
			title: "deploy",
			startCreateDeploy: info(
				`Запущен автоматический процесс создания файла ${filename}`
			),
			created: info(
				`Настройте файл синхронизации "${filename}" и запустите команду повторно`
			),
			end: (deployData) =>
				success(
					`Загружено в \x1b[35m${deployData.hostname}:${deployData.destination}\x1b[32m`
				),
			startDefault: (mode) =>
				info(`Запущена выгрузка с "default" значением "${mode}"`),
			empty: warn(`Не существует файл ${filename}`),
			emptyData: (mode) =>
				error(
					`Не найдена настройка для режима "${mode}". Проверьте, что объект "${mode}" правильно настроен в конфиге "${filename}"`
				),
			errorSettings: (mode) =>
				error(`Неверно настроен файл "${filename}" для режима "${mode}"`),
		},
	},
};
